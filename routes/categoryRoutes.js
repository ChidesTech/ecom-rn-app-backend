...
router.get("/:id", async(req, res)=>{
    ...
    if(!category){
    res.status(500).json({success:false, message: "The category wasn't found"})
    }
...
router.delete("/:id", (req, res) => {
    ...
    Category.findOneAndDelete(req.params.id).then(category => {
        if (category) {
            return res.status(200).json({ success: true, message: "the category was successfully deleted" })
        } else {
